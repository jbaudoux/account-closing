...
class TestAccountCutoffAccrualPicking(TransactionCase):
    ...
    def test_accrued_revenue(self):
        ...
        self.assertTrue(len(lines) == 2, '2 lines should be found to calculate cutoff')
        account_mapping = cutoff._get_account_mapping()
        l = cutoff._prepare_lines(lines[0], account_mapping)
        self.assertTrue(l['cutoff_amount'] == 560, 'Line 0 cutoff amount incorrect')
        l = cutoff._prepare_lines(lines[1], account_mapping)
        self.assertTrue(l['cutoff_amount'] == 180, 'Line 1 cutoff amount incorrect')
    ...
    def test_accrued_expense(self):
        ...
        self.picking.pack_operation_product_ids.write({'qty_done': 4.0})
        self.picking.do_new_transfer()
        # TODO : Find how to get the stock moving
        #pp.pprint(self.po.order_line.read())
        lines = cutoff.get_lines_for_cutoff()
        self.assertTrue(len(lines)==1, '1 lines should be found to calculate expense cutoff')
        account_mapping = cutoff._get_account_mapping()
        l = cutoff._prepare_lines(lines[0], account_mapping)
        #pp.pprint(l)
        #self.assertTrue(l['cutoff_amount']==560, 'Line 0 cutoff amount incorrect')
